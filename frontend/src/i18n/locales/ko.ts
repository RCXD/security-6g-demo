...
export const ko: Messages = {
  meta: {
    ...
    description:
      '차량 네트워크(VANET)에서 차분(differential) 특징과 오토인코더(autoencoder) 프로파일링(profiling)으로 위치 스푸핑(location spoofing) 및 변형(variation) 공격을 탐지하는 인터랙티브 연구 데모입니다.',
  },
  lang: {
    label: '언어',
  ...
  hero: {
    ...
    stats: {
      ...
      features: '핵심 차분 특징 수',
      zeroDay: '미지의 공격에 대한 오토인코더(autoencoder) 프로파일링(profiling)',
    },
  },
  overview: {
    eyebrow: '핵심 아이디어',
  ...
  contributions: {
    ...
    subtitle:
      '선행 연구와 실험을 통해 위치 스푸핑(location spoofing) 탐지의 병목을 확인하고, 이동성(mobility) 기반 차분(differential) 특징을 제안했으며, 공격 변형(variation)과 미지 패턴에 대한 프로파일링(profiling) 및 강건성(robustness)을 평가했습니다.',
    problem: {
      title: '위치 스푸핑 탐지에서 확인된 접근별 한계',
      body:
        '위치 무결성을 지키려면 크게 세 갈래—지도 학습(supervised learning) 분류, 원시 좌표 특징, 공격 변형(variation) 대응—마다 다른 병목이 있습니다. IEEE Access 2023은 이를 출발점으로 삼아 차분 특징과 프로파일링을 제안했습니다.',
      bullets: [
        '지도 학습(supervised learning): 위조 궤적 레이블이 필요해 알려진 스푸핑 패턴에는 강하지만, 제로데이(zero-day) 변형에는 취약합니다.',
        '원시 좌표(Basic 특징): 구현이 단순하지만 어려운 공격에서 성능이 무너집니다. SVM은 검증 94%대에서 테스트 63%로 급락합니다.',
        '기존 VeReMi 기반 연구: 표준 공격 유형 위주로 다뤄, 오프셋 스윕·위치 고정 등 공격자가 좌표를 바꾸는 변형(variation)을 충분히 반영하지 못했습니다.',
      ],
    },
    solution: {
      title: '차분(differential) Ext 특징 + 2-BSM 경량 탐지',
      body:
        'IEEE Access 2023은 연속된 두 개의 Basic Safety Message(BSM)만으로 계산하는 차분(differential) Ext 특징 집합을 제안합니다.',
      bullets: [
        'Ext 특징(Dᵗ, dᵗ, Δt, κᵗ)은 이동성(mobility) 제약과 불일치를 인코딩해, 보고된 움직임이 물리적으로 타당한지 O(1) 시간에 검사합니다.',
        'Basic에서 Ext로 바꾸면 모든 모델 성능이 향상됩니다. MLP는 99.1% 테스트 정확도, SVM도 63%에서 94.7%로 회복합니다.',
        '연속된 두 메시지(Sᵗ⁻¹, Sᵗ)만 필요하고 쌍별 비교가 없어, 차량 내 실시간(real-time) 탐지에 적합합니다.',
      ],
    },
    adversarial: {
      title: '공격 변형(variation)과 미지 스푸핑에 대한 프로파일링(profiling)',
      body:
        '벤치마크 정확도를 넘어, 공격자가 좌표를 어떻게 바꾸는지 모델링하고, 정상 트래픽만으로 학습한 탐지기가 새로운 스푸핑을 잡을 수 있는지—제로데이(zero-day) 강건성(robustness)—을 평가합니다.',
      bullets: [
        'IEEE Access는 기준 VeReMi 유형을 넘어 오프셋 스윕, 위치 고정, 무작위 산포 등 좌표 조작 시나리오를 정의해 탐지기를 스트레스 테스트합니다.',
        'ICMLA 2023은 세 가지 오토인코더(autoencoder) 프로파일러를 지도 학습(supervised learning) 모델과 표준·변형(variation) 공격에서 비교합니다. 프로파일링(profiling)은 위조 레이블 없이도 지도 모델과 맞먹거나 더 나은 성능을 냅니다.',
        '프로파일링은 스푸핑 시그니처를 외우는 대신 학습된 정상 운동에서 벗어난 편차를 탐지합니다. 지능적·미지의 회피(evasion) 공격에 대한 강건성(robustness)의 핵심입니다.',
      ],
    },
    venues: {
      title: '연구가 발표·게재된 곳',
  ...
  results: {
    ...
    heatmapTitle: '공격 변형(variation)에 대한 강건성(robustness) (복원된 히트맵)',
    heatmapBody:
  ...
  research: {
    ...
    abstracts: {
      '10.1109/ACCESS.2023.3241236':
        '위치 스푸핑(location spoofing)과 그 변형(variation) 공격을 안정적으로 탐지하기 위한 데이터 기반 방법론을 제시합니다. 이동성(mobility) 제약과 불일치를 검사하는 새로운 차분(differential) 특징 집합으로 최대 99.1%의 탐지 정확도를 달성했으며, 제로데이(zero-day) 탐지를 위한 오토인코더(autoencoder) 기반 프로파일링(profiling) 접근도 함께 제안합니다.',
      '10.1109/ICMLA58977.2023.00085':
        '정상 데이터로 프로파일(profiling)을 구축하는 오토인코더(autoencoder) 기반 탐지가, 지능적이고 이전에 보지 못한 공격에도 강건한지(robust)를 실증적으로 평가합니다. 세 가지 오토인코더 구조를 지도 학습(supervised learning) 모델과 비교하고, 표준 및 변형(variation) 공격에 대한 성능을 분석합니다.',
    },
  },
  team: {
    eyebrow: '연구진',
