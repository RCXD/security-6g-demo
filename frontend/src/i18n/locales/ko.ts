...
export const ko: Messages = {
  meta: {
    ...
    description:
      '차량 애드혹 네트워크(VANET)에서 차분 특징(differential feature)과 오토인코더(autoencoder) 프로파일링(profiling)으로 위치 스푸핑(location spoofing)을 탐지하는 인터랙티브 연구 데모입니다.',
  },
  lang: {
    label: '언어',
    chooseTitle: '언어 선택',
    chooseSubtitle: '상단 메뉴에서 언제든 다시 변경하실 수 있습니다.',
    continue: '시작하기',
    names: {
      en: 'English',
      ko: '한국어',
  ...
  hero: {
    ...
    subtitle:
      '연속된 안전 메시지(Basic Safety Message, BSM) 두 개만으로 위조 GPS를 가려내는 방법을 소개합니다. 차분 특징(differential feature)으로 탐지 정확도는 최대 {accuracy}%에 이릅니다.',
    ctaAttacks: '공격 유형 보기',
    ctaResults: '실험 결과 보기',
    stats: {
  ...
  overview: {
    ...
    subtitle:
      '자율주행차·드론·무인 체계가 늘어날수록, 보고된 좌표 하나를 믿을 수 있는지가 곧 안전과 작전 성패를 가르는 문제가 됩니다. 차량 애드혹 네트워크(VANET)에서 위치는 충돌 회피·차선 변경의 근거가 되고, 그 좌표를 속이면 사고로 직결됩니다. 이번 연구는 최소한의 메시지 정보만으로도 그런 위조를 안정적으로 잡아내는 방법에 초점을 맞춥니다.',
    cards: {
      bsm: {
        title: 'Basic Safety Message (BSM)',
        body: '차량은 위치·속도·시각을 초당 약 10번 방송합니다. 이른바 BSM입니다. 공격자는 이 메시지에 가짜 좌표를 끼워 넣을 수 있습니다.',
      },
      sequence: {
        title: '2-시퀀스(2-sequence) 탐지',
        body: '연속 메시지 두 개(Sᵗ⁻¹, Sᵗ)면 충분합니다. 차분 특징 Dᵗ, dᵗ, Δt, κᵗ로 “이 움직임이 물리적으로 자연스러운지”를 봅니다. 한 장의 스냅샷이 아니라, 바로 앞뒤 한 걸음을 비교하는 식입니다.',
      },
      realtime: {
        title: '경량·실시간',
        body: '특징 계산 비용은 O(1)입니다. 차량끼리 일일이 비교할 필요도 없습니다. 차량 안에서도 실시간으로 돌리기 쉬운 구조입니다.',
      },
    },
    compare: {
      ...
      static: {
        ...
        sampleCaption:
          '이전 평가는 종종 각 BSM을 따로 떨어진 정적 샘플로 봅니다. 시간 맥락 없이 (x, y) 벡터 하나만 쓰는 식입니다.',
        spoofCaption:
          '좌표 하나만 보면 위조 값도 그럴듯해 보일 수 있습니다. Basic(원시 좌표) feature에서는 특히 그렇습니다.',
        limitCaption:
          '연속 메시지가 없으면 움직임이 자연스러운지 확인할 수 없습니다. 이상한 점프도 그대로 숨습니다.',
        missedCaption:
          '스냅샷 하나만 보는 정적 테스트는, 시간이 지나야 드러나는 스푸핑을 놓칩니다.',
        summaryCaption:
          '정적 VeReMi 샘플만으로는 안정적이고 높은 탐지율을 내기 어려웠던 이유입니다.',
        missedBadge: '미탐',
      },
      sequence: {
        title: '2-시퀀스(2-sequence) 데이터셋 (본 연구)',
        streamCaption:
          'VeReMi BSM 로그를 연속 쌍(Sᵗ⁻¹, Sᵗ)으로 다시 묶습니다. 정적 샘플의 흐름이 시간 축이 있는 데이터셋으로 바뀝니다.',
        pairCaption:
          '학습 단위가 메시지 하나가 아니라 2-시퀀스 윈도우(window)입니다. 보는 대상이 스냅샷에서 움직임으로 바뀝니다.',
        featureCaption:
          '쌍에서 Dᵗ, dᵗ, Δt, κᵗ를 뽑아, 그 한 걸음이 물리적으로 타당한지 숫자로 남깁니다.',
        detectCaption:
          '위조된 점프는 두 메시지 사이의 이동 제약(mobility constraint)을 깨뜨립니다. 각 점이 멀쩡해 보여도 쌍으로는 잡힙니다.',
        trainCaption:
          '이 2-시퀀스 Ext feature로 학습하면 최대 99.1%까지 갑니다. 정적 파이프라인으로는 내기 어려웠던 수치입니다.',
        detectedBadge: '탐지',
      },
    },
  },
  contributions: {
    ...
    title: '왜 지금, 위치 무결성인가',
    subtitle:
      '논문 속 VeReMi 실험만으로 끝나지 않는 이야기입니다. 최근 전장과 무인 체계가 보여 주듯, 위치·항법·통신 네트워크의 신뢰는 이미 민간 안전과 안보를 가르는 문제로 올라와 있습니다.',
    context: {
      title: '드론·무인 체계 시대, 위치가 흔들리면 전체가 흔들리는 이유',
      body:
        '우크라이나와 러시아 전쟁, 중동에서의 미국과 이란 관련 긴장만 봐도 드론(UAV)과 네트워크 중심 작전이 전장의 기본 문법이 됐다는 점을 알 수 있습니다. 공중에서 움직이는 플랫폼이든, 지상의 차량 네트워크든, 보고된 좌표가 틀리면 충돌 회피·표적 식별·군집 운용 같은 상위 판단이 한꺼번에 무너집니다. 위치 스푸핑(location spoofing)은 그래서 “논문용 공격”이 아니라, 지금 현장에서 커지고 있는 위협 모델에 가깝습니다.',
      bullets: [
        '우러 전쟁에서 드론·전자전·항법 교란이 일상화되며, GPS·통신 신뢰의 중요성이 재확인됐습니다.',
        '미국과 이란 관련 긴장 속에서도 무인기·방공·네트워크 연동의 취약점이 안보 이슈로 떠올랐습니다.',
        '같은 논리는 VANET·자율주행·스마트 모빌리티에도 적용됩니다. 가짜 위치 한 줄이 연쇄 사고를 부를 수 있습니다.',
      ],
    },
    prior: {
      title: '기존 연구: 정적 샘플(static sample) 평가',
      body:
        '위치 스푸핑 탐지 선행 연구 상당수는 개별 BSM 스냅샷에서 뽑은 정적 특징 벡터로 탐지기(detector)를 평가합니다. VeReMi가 메시지당 한 줄로 저장되는 방식과 비슷하지만, 스푸핑을 드러내는 시간 정보는 사라집니다.',
      bullets: [
        '각 메시지를 시간 축 없이 순간 좌표나 Basic feature로만 표현합니다.',
    ...
    sequence: {
      ...
      body:
        '같은 VeReMi 샘플을 연속 2-시퀀스 윈도우로 다시 묶고, Ext 차분 특징을 만든 뒤 학습합니다. 탐지는 고립된 좌표가 아니라 “움직임이 일관되는가”의 문제가 됩니다.',
      bullets: [
        '차량 방송 스트림에서 연속 메시지 쌍(Sᵗ⁻¹, Sᵗ)을 만듭니다.',
        'Dᵗ, dᵗ, Δt, κᵗ를 계산해 변위, 이동 거리, 시간 간격, 이동 타당성(MPC)을 담습니다.',
        '2-시퀀스 Ext 데이터로 MLP / RF / XGB / SVM을 학습하고, Basic feature는 제거 실험(ablation) 기준선으로 둡니다.',
      ],
    },
    impact: {
      title: '학습 결과: Basic → Ext',
      body:
        '데이터셋을 2-시퀀스 차분 특징 중심으로 다시 만들면 평가한 모델이 모두 좋아집니다. IEEE Access 2023에 나온 어려운 공격 유형·파라미터 변형에서도 효과가 큽니다.',
      bullets: [
  ...
  attacks: {
    ...
    items: {
      ...
      constant_offset: {
        ...
        description:
          '실제 위치에 고정 오프셋(Δx=+250, Δy=−150)을 더한 값을 매번 알립니다. 궤적 모양은 비슷하지만 통째로 밀려 있습니다.',
      },
      random: {
        name: 'Random',
        short: '임의 위치',
        description:
          '넓은 영역에서 좌표를 무작위로 뽑습니다. 보고 위치가 지도 여기저기에 흩어집니다.',
      },
      random_offset: {
        name: 'Random offset',
        short: '임의 Δ shift',
      ...
      eventual_stop: {
        ...
        short: '정지한 듯 고정',
        description:
          '시간이 갈수록 “멈춘 척”할 확률을 높입니다. 보고 위치가 점점 한곳에 붙습니다.',
      },
    },
  },
  results: {
    ...
    ext: 'Ext',
    accuracyExtNote:
      'Ext feature를 쓰면 MLP는 99.1%에 이르고, SVM도 94.7%까지 회복합니다.',
    accuracyBasicNote:
      'Basic feature만 쓰면 SVM은 63%까지 떨어지고, MLP도 테스트에서 91.4%로 낮아집니다.',
  ...
  research: {
    ...
    title: '논문',
    subtitle:
      '아래 동료 심사(peer-reviewed) 논문의 결과를 재구성한 데모입니다. PDF는 배포하지 않으며 DOI 링크만 제공합니다.',
    abstracts: {
      '10.1109/ACCESS.2023.3241236':
        '위치 스푸핑과 그 변형을 안정적으로 탐지하는 데이터 기반 방법을 제안합니다. 이동 제약과 불일치를 보는 새 차분 특징으로 최대 99.1% 탐지 정확도를 냈고, 제로데이(zero-day)용 오토인코더(autoencoder) 프로파일링도 함께 제안했습니다.',
      '10.1109/ICMLA58977.2023.00085':
        '정상 데이터로 프로파일을 만드는 오토인코더 탐지가, 이전에 보지 못한 지능형 공격에도 강건한지 실증적으로 평가합니다. 오토인코더 구조 세 가지를 지도 학습(supervised learning) 모델과 비교하고, 표준·변형 공격에서의 성능을 살폈습니다.',
    },
  },
  team: {
    eyebrow: '연구진',
    ...
    funding:
      '본 연구는 일부 IITP(과학기술정보통신부)의 지원을 받았습니다. 과제명 “상시적 보안 품질 보장을 위한 6G 자율 보안 설계 기반 기술 연구”, 과제번호 2021-0-00796.',
    roles: {
      'Jinoh Kim': '연구책임자 / 교수',
      'Chiho Kim': '제1저자 / 연구원',
      'Dongeun Lee': '공동저자',
  ...
  footer: {
    ...
    right: '기록된 실험 결과를 재구성한 자료입니다 · VeReMi dataset.',
  },
}
